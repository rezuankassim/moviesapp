<div class="mt-8">
    <a href="{{ route('movies.show', $movie['id']) }}">
        <img src="{{ config('services.tmdb.image_url').$movie['poster_path'] }}" alt="poster" class="hover:opacity-75 transition ease-in-out duration-150">
    </a>
    <div class="mt-2">
        ...
        <div class="text-gray-400 text-sm">
            @foreach ($movie['genre_ids'] as $genre_id)
                <a href="#" class="hover:underline">{{ $genres[$genre_id] }}</a>{{ $loop->last ? '' : ', ' }}
            @endforeach
        </div>
    </div>
</div>